...
7 независимых скиллов + 1 мета-оркестратор. Устанавливаются вместе, работают по триггерам в чате.

> **Принцип отбора:** в репо только то, что **решается через API** или фиксится **вручную в админке**. Скиллы, которые «детектят, но починить нельзя» — не нужны.
...
| **[`horoshop-consistency`](horoshop-consistency/)** | Конфликты между текстом и характеристиками: материал/страна/цвет/размер не совпадают, discount math, плейсхолдеры | «противоречия в карточках», «характеристики не сходятся» |
| **[`horoshop-marketing-psych`](horoshop-marketing-psych/)** | Усиление текстов карточек психологическими приёмами (scarcity, anchoring, social proof) через `description`/`short_description`. Не пишет `icons[]` — кастомные стикеры через API создать нельзя | «маркетинг-приёмы в карточки», «продающий стиль» |
...
### Вариант 1 — одной командой (все 8 скиллов сразу)

```bash
curl -fsSL https://raw.githubusercontent.com/IgorShutko/horoshop-claude-skill/main/install.sh | bash
```

Установит все 8 скиллов в `~/.claude/skills/horoshop-*` + Python-зависимости.
...
├── horoshop-marketing-psych/   # психо-приёмы для конверсии (через текст)
├── install.sh                  # Устанавливает все 8 скиллов
