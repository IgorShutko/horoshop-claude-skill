...
7 незалежних скілів + 1 мета-оркестратор. Встановлюються разом, працюють за тригерами в чаті.

> **Принцип відбору:** в репо тільки те, що **вирішується через API** або фіксаться **вручну в адмінці**. Скіли, які «детектять, але починити не можна» — не потрібні.
...
| **[`horoshop-consistency`](horoshop-consistency/)** | Конфлікти між текстом і характеристиками: матеріал/країна/колір/розмір не збігаються, discount math, плейсхолдери | «суперечності в картках», «характеристики не сходяться» |
| **[`horoshop-marketing-psych`](horoshop-marketing-psych/)** | Підсилення текстів карток психологічними прийомами (scarcity, anchoring, social proof) через `description`/`short_description`. Не пише `icons[]` — кастомні стикери через API створити не можна | «маркетинг-прийоми в картки», «продаючий стиль» |
...
### Варіант 1 — однією командою (всі 8 скілів одразу)

```bash
curl -fsSL https://raw.githubusercontent.com/IgorShutko/horoshop-claude-skill/main/install.sh | bash
```

Встановить усі 8 скілів у `~/.claude/skills/horoshop-*` + Python-залежності.
...
├── horoshop-marketing-psych/   # психо-прийоми для конверсії (через текст)
├── install.sh                  # Встановлює всі 8 скілів
